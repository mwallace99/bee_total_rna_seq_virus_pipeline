...
# Array width for the grouped PBS-array stages (13 blastn, 15 diamond, 17 rdrp):
# BLAST_NGROUPS subjobs, each processing a slice of the samples (sample k, k+G,
# ...). For stage 13 each subjob also warms the ~706 GB nt cache once, so fewer
# groups = more samples per node = the warm cache is reused more (cheaper, less
# Lustre load) but less wall-clock parallelism. ~4-5 suits ~20 samples.
# submit_all.sh caps this at the sample count and at Gadi's 10-element array limit.
BLAST_NGROUPS=5
